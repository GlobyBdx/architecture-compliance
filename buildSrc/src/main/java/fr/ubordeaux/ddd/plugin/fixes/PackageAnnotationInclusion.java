...
import fr.ubordeaux.ddd.plugin.fixes.utils.PackageAnnotationInclusionProcessor;
...
public final class PackageAnnotationInclusion extends AbstractAnnotationInclusion {
    public PackageAnnotationInclusion(String annotation) {
        super(annotation);
    }
    ...
    public void use(String name, Description description, Project project) {
        if (description == null) {
            ...
            ProcessorLauncher.launch(
                    QuickFixUtils.getPackageInfoPath(name, project), project.getFullPath(),
                    new PackageAnnotationInclusionProcessor(
                            QuickFixUtils.getClassPackageName(name, project), this.getAnnotation(), project));
        }
        else {
            AnnotationManager.manage(
                    description, QuickFixUtils.getPackageInfoName(name, project), this.getAnnotation(), true);
            description.update();
        }
    }
}
