...
public class MethodAnnotationRemovalProcessor extends AbstractProcessor<CtMethod<?>> {
    private final String name;
    private final String annotation;
    private final Project project;

    public MethodAnnotationRemovalProcessor(String name, String annotation, Project project) {
        this.name = name;
    ...
    public void process(CtMethod<?> element) {
        if (element.getSimpleName().compareTo(this.name) == 0) {
            ...
            element.removeAnnotation(element.getAnnotation(typeReference));
        }
    }
}
